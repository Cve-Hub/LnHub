...
pcall(function()
   userinfo = HttpService:JSONDecode(readfile("somexhub.txt"));
end)
...
local function SaveInfo()
   ...
   userinfo["user"] = user
   userinfo["tag"] = tag
   writefile("somexhub.txt", HttpService:JSONEncode(userinfo));
end
...
local NillSC = Instance.new("ScreenGui")
NillSC.Name = _G.Key .."," .. _G.Discord
NillSC.Parent = game.CoreGui
NillSC.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

game:GetService("UserInputService").InputBegan:connect(function(inputObject, gameProcessedEvent)
   if inputObject.KeyCode == Enum.KeyCode.RightControl then
      wait()
      NillSC.Enabled = not NillSC.Enabled
   end
end)

function Nill:Window(text,maincolor)
   ...
   return ServerHold
	...
	wait(3)
	local win = Nill:Window("nill")

	local serv = win:Server("V.0", "")

	local AutoFram = serv:Channel("Auto")
	
	AutoFram:Label("Auto ")
	AutoFram:Toggle("Auto ",_G.AutoFarm,function(vu)
		_G.AutoFarm = vu
...
Nill:Notification("Notification", "Button", "Okay!")
end)
